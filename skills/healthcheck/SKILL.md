---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for FlowHelm deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, FlowHelm cron scheduling for periodic checks, or version status checks on a machine running FlowHelm (laptop, workstation, Pi, VPS).
---

# FlowHelm Host Hardening

## Overview

Assess and harden the host running FlowHelm, then align it to a user-defined risk tolerance without breaking access. Use FlowHelm security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim FlowHelm changes the host firewall, SSH, or OS updates; it does not.
...
5. FlowHelm gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if FlowHelm can access sensitive data.
...
### 2) Run FlowHelm security audits (read-only)

As part of the default read-only checks, run `flowhelm security audit --deep`. Only offer alternatives if the user requests them:

1. `flowhelm security audit` (faster, non-probing)
2. `flowhelm security audit --json` (structured output)

Offer to apply FlowHelm safe defaults (numbered):

1. `flowhelm security audit --fix`

Be explicit that `--fix` only tightens FlowHelm defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check FlowHelm version/update status (read-only)

As part of the default read-only checks, run `flowhelm update status`.
...
- Credential hygiene notes (location of FlowHelm creds, prefer disk encryption)
...
- FlowHelm security audit (re-run)
...
After FlowHelm install or first hardening pass, run at least one baseline audit and version check:

- `flowhelm security audit`
- `flowhelm security audit --deep`
- `flowhelm update status`

Ongoing monitoring is recommended. Use the FlowHelm cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `flowhelm cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `flowhelm update status`
...
Before creating, `flowhelm cron list` and match on exact `name`. If found, `flowhelm cron edit <id> ...`.
If not found, `flowhelm cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `flowhelm update status` (preferred for source checkouts and channels)
2. `npm view flowhelm version` (published npm version)

## FlowHelm command accuracy

Use only supported commands and flags:

- `flowhelm security audit [--deep] [--fix] [--json]`
- `flowhelm status` / `flowhelm status --deep`
- `flowhelm health --json`
- `flowhelm update status`
- `flowhelm cron add|list|runs|run`

Do not invent CLI flags or imply FlowHelm enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by FlowHelm compaction:
