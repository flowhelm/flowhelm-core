...
This document describes how the FlowHelm ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`flowhelm acp` exposes an ACP agent over stdio and forwards prompts to a running
FlowHelm Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `flowhelm acp` and speaks ACP messages over stdio.
...
`flowhelm acp` resolves the Gateway URL and auth from CLI flags or config:
