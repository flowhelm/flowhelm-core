...
const defaultTools = createFlowHelmCodingTools({ senderIsOwner: true });
...
describe("createFlowHelmCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: FlowHelmConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createFlowHelmCodingTools({
    ...
    const anthropicTools = createFlowHelmCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: FlowHelmConfig = {
      tools: {
        exec: {
    ...
    const allowed = createFlowHelmCodingTools({
    ...
    const denied = createFlowHelmCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createFlowHelmCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
