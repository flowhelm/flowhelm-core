...
type WebSearchConfig = NonNullable<FlowHelmConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: FlowHelmConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("flowhelm configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.flowhelm.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://flowhelm.ai",
      "X-Title": "FlowHelm Web Search",
    },
