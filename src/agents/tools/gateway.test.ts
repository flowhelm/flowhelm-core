...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    flowhelm: process.env.FLOWHELM_GATEWAY_TOKEN,
    flowhelm_bot: process.env.FLOWHELM_BOT_GATEWAY_TOKEN,
  };
  ...
  beforeEach(() => {
    ...
    delete process.env.FLOWHELM_BOT_GATEWAY_TOKEN;
  });

  afterAll(() => {
    if (envSnapshot.flowhelm === undefined) {
      delete process.env.FLOWHELM_GATEWAY_TOKEN;
    } else {
      process.env.FLOWHELM_GATEWAY_TOKEN = envSnapshot.flowhelm;
    }
    if (envSnapshot.flowhelm_bot === undefined) {
      delete process.env.FLOWHELM_BOT_GATEWAY_TOKEN;
    } else {
      process.env.FLOWHELM_BOT_GATEWAY_TOKEN = envSnapshot.flowhelm_bot;
    }
  });
  ...
  it("uses FLOWHELM_GATEWAY_TOKEN for allowlisted local overrides", () => {
    process.env.FLOWHELM_GATEWAY_TOKEN = "env-token";
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.FLOWHELM_GATEWAY_TOKEN = "local-env-token";
    process.env.FLOWHELM_BOT_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
