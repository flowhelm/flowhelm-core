...
function resolveBrowserBaseUrl(params: {
  ...
  if (!resolved.enabled) {
    throw new Error(
      "Browser control is disabled. Set browser.enabled=true in ~/.flowhelm/flowhelm.json.",
    );
  }
  return undefined;
}
...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control browser (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      'Profiles: "chrome" for existing tabs via extension relay; "flowhelm" for isolated browser.',
      'Always use profile="chrome" if user mentions Chrome extension/relay/toolbar.',
      'Chrome relay requires an "attached" tab (user clicks icon, badge ON).',
      "Keep same tab using targetId from snapshot in subsequent actions.",
      'Use snapshot(refs="aria") for stable refs. refs="role" is default.',
      "Prefer snapshot+act for UI automation. Avoid act:wait.",
      `target: sandbox|host|node. Default: ${targetDefault}. ${hostHint}`,
    ].join(" "),
        ...
        case "act": {
          ...
          try {
            ...
            if (msg.includes("404:") && msg.includes("tab not found") && profile === "chrome") {
              ...
              if (!tabs.length) {
                throw new Error(
                  "No Chrome tabs are attached via the FlowHelm Browser Relay extension. Click the toolbar icon on the tab you want to control (badge ON), then retry.",
                  { cause: err },
                );
              }
              throw new Error(
