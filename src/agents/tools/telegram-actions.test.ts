...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): FlowHelmConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as FlowHelmConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): FlowHelmConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as FlowHelmConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: FlowHelmConfig, accountId = "media") {
    await handleTelegramAction(
