...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "flowhelm" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "FlowHelm", url: "https://flowhelm.ai/docs", content: "docs" },
            ],
          }),
    ...
    const result = await tool?.execute?.("call-1", { query: "latest flowhelm release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://flowhelm.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://flowhelm.ai/docs"]);
