...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("flowhelm-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("flowhelm-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("flowhelm-sbx-");
