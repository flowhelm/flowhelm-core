...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("flowhelm-ws-", async (workspaceDir) => {
      await withTempDir("flowhelm-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createFlowHelmCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("flowhelm-ws-", async (workspaceDir) => {
      await withTempDir("flowhelm-override-", async (overrideDir) => {
        const tools = createFlowHelmCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("flowhelm-ws-", async (workspaceDir) => {
      const cfg: FlowHelmConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createFlowHelmCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-flowhelm.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("flowhelm-sandbox-", async (sandboxDir) => {
      await withTempDir("flowhelm-workspace-", async (workspaceDir) => {
        ...
        const tools = createFlowHelmCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
