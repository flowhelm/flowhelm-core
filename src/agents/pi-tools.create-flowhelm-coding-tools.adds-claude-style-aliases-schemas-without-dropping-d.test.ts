...
describe("createFlowHelmCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-read-"));
    try {
      ...
      const contents = "Hello from flowhelm read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createFlowHelmCodingTools({ sandbox });
