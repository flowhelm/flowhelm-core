...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "flowhelm.sandbox=1");
  args.push("--label", `flowhelm.sessionKey=${params.scopeKey}`);
  args.push("--label", `flowhelm.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `flowhelm.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "flowhelm.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`flowhelm sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`flowhelm sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("flowhelm sandbox recreate --all");
}
