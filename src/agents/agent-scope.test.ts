...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: FlowHelmConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/flowhelm" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: FlowHelmConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: FlowHelmConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: FlowHelmConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: FlowHelmConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: FlowHelmConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/flowhelm");
  });

  it("uses FLOWHELM_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "flowhelm-home");
    vi.stubEnv("FLOWHELM_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as FlowHelmConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".flowhelm", "workspace"));
  });

  it("uses FLOWHELM_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "flowhelm-home");
    vi.stubEnv("FLOWHELM_HOME", home);
    // Clear state dir so it falls back to FLOWHELM_HOME
    vi.stubEnv("FLOWHELM_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as FlowHelmConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".flowhelm", "agents", "main", "agent"));
  });
});
