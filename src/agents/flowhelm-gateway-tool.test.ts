...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createFlowHelmTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-test-"));

    try {
      await withEnvAsync(
        { FLOWHELM_STATE_DIR: stateDir, FLOWHELM_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: flowhelm --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/flowhelm" } }\n}\n';
