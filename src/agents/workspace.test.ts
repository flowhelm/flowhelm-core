...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses FLOWHELM_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      FLOWHELM_HOME: "/srv/flowhelm-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/flowhelm-home"), ".flowhelm", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".flowhelm", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("flowhelm-workspace-");
