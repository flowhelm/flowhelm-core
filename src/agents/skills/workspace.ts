...
function resolveSkillsLimits(config?: FlowHelmConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "flowhelm-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "flowhelm-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveFlowHelmMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: FlowHelmConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`flowhelm skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
