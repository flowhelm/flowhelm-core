...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.FLOWHELM_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".flowhelm", `workspace-${profile}`);
  }
  return path.join(home, ".flowhelm", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".flowhelm";
