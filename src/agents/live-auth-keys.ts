...
const GOOGLE_LIVE_SINGLE_KEY = "FLOWHELM_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "FLOWHELM_LIVE_ANTHROPIC_KEY",
    listVar: "FLOWHELM_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "FLOWHELM_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `FLOWHELM_LIVE_${base}_KEY`;
