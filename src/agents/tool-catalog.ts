...
type CoreToolDefinition = {
  ...
  includeInFlowHelmGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const flowhelmTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInFlowHelmGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:flowhelm": flowhelmTools,
