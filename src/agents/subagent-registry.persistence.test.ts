...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["FLOWHELM_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-subagent-"));
  ...
  it("uses isolated temp state when FLOWHELM_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "flowhelm-test-state"));
  });
});
