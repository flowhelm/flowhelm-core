...
import { loadFlowHelmPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `flowhelm-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.FLOWHELM_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "flowhelm.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.FLOWHELM_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadFlowHelmPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadFlowHelmPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.FLOWHELM_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.FLOWHELM_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadFlowHelmPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadFlowHelmPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadFlowHelmPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.FLOWHELM_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ FLOWHELM_STATE_DIR: stateDir, FLOWHELM_BOT_STATE_DIR: undefined }, () => {
