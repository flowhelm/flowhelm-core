...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: FlowHelmPluginDefinition;
  register?: FlowHelmPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as FlowHelmPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as FlowHelmPluginDefinition;
...
export function loadFlowHelmPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverFlowHelmPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "flowhelm/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "flowhelm/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: FlowHelmPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as FlowHelmPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
