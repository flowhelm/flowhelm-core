...
describe("shared/frontmatter", () => {
  ...
  test("resolveFlowHelmManifestBlock parses JSON5 metadata and picks flowhelm block", () => {
    const frontmatter = {
      metadata: "{ flowhelm: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveFlowHelmManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveFlowHelmManifestBlock returns undefined for invalid input", () => {
    expect(resolveFlowHelmManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveFlowHelmManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveFlowHelmManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
