...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@flowhelm/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@flowhelm/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@flowhelm/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@flowhelm/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @flowhelm/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@flowhelm/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@flowhelm/plugin-alpha@1.2.3",
        installPath: "/tmp/flowhelm/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@flowhelm/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @flowhelm/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@flowhelm/plugin-alpha@latest",
      true,
      "/tmp/flowhelm/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@flowhelm/plugin-alpha@latest",
      installPath: "/tmp/flowhelm/extensions/alpha",
      version: "1.2.3",
