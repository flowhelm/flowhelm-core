    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["flowhelm memory status", "Show index and provider status."],
          ["flowhelm memory index --force", "Force a full reindex."],
          ['flowhelm memory search --query "deployment notes"', "Search indexed memory entries."],
          ["flowhelm memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.flowhelm.ai/cli/memory")}\n`,
    );
