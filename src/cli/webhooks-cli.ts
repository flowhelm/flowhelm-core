...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.flowhelm.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + FlowHelm hooks")
    ...
    .option("--hook-url <url>", "FlowHelm hook URL")
    .option("--hook-token <token>", "FlowHelm hook token")
