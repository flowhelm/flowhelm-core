    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["flowhelm nodes status", "List known nodes with live status."],
          ["flowhelm nodes pairing pending", "Show pending node pairing requests."],
          ['flowhelm nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["flowhelm nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.flowhelm.ai/cli/nodes")}\n`,
    );
