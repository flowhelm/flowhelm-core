...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("flowhelm node install"), formatCliCommand("flowhelm node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("flowhelm node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    FLOWHELM_LOG_PREFIX: baseEnv.FLOWHELM_LOG_PREFIX ?? "node",
