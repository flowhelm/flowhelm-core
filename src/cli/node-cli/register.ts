    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "flowhelm node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["flowhelm node status", "Check node host service status."],
          ["flowhelm node install", "Install the node host service."],
          ["flowhelm node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.flowhelm.ai/cli/node")}\n`,
    );
