...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "flowhelm-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "flowhelm-bundled-skills-test-"));
    process.env.FLOWHELM_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
