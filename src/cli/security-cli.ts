...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["flowhelm security audit", "Run a local security audit."],
          ["flowhelm security audit --deep", "Include best-effort live Gateway probe checks."],
          ["flowhelm security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["flowhelm security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.flowhelm.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("FlowHelm security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("flowhelm security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("flowhelm security audit --fix")}`));
        if (!fixResult) {
