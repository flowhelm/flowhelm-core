...
${formatHelpExamples([
  ['flowhelm agent --to +15555550123 --message "status update"', "Start a new session."],
  ['flowhelm agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'flowhelm agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'flowhelm agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['flowhelm agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'flowhelm agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.flowhelm.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.flowhelm.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['flowhelm agents set-identity --agent main --name "FlowHelm" --emoji "🦞"', "Set name + emoji."],
  ["flowhelm agents set-identity --agent main --avatar avatars/flowhelm.png", "Set avatar path."],
  [
    "flowhelm agents set-identity --workspace ~/.flowhelm/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "flowhelm agents set-identity --identity-file ~/.flowhelm/workspace/IDENTITY.md --agent main",
