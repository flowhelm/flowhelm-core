...
beforeEach(() => {
  ...
  originalHideBanner = process.env.FLOWHELM_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.FLOWHELM_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.FLOWHELM_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("flowhelm");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "flowhelm", "status", "--debug"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "flowhelm", "configure"],
    });
  ...
  it("loads plugin registry for onboard command", async () => {
    await runCommand({
      parseArgv: ["onboard"],
      processArgv: ["node", "flowhelm", "onboard"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "flowhelm", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "flowhelm", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "flowhelm", "--version"],
    });
  ...
  it("hides banner when FLOWHELM_HIDE_BANNER is truthy", async () => {
    process.env.FLOWHELM_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "flowhelm", "status"],
    });
