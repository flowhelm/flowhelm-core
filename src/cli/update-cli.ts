...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update FlowHelm and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["flowhelm update", "Update a source checkout (git)"],
        ["flowhelm update --channel beta", "Switch to beta channel (git + npm)"],
        ["flowhelm update --channel dev", "Switch to dev channel (git + npm)"],
        ["flowhelm update --tag beta", "One-off update to a dist-tag or version"],
        ["flowhelm update --dry-run", "Preview actions without changing anything"],
        ["flowhelm update --no-restart", "Update without restarting the service"],
        ["flowhelm update --json", "Output result as JSON"],
        ["flowhelm update --yes", "Non-interactive (accept downgrade prompts)"],
        ["flowhelm update wizard", "Interactive update wizard"],
        ["flowhelm --update", "Shorthand for flowhelm update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run flowhelm update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.flowhelm.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["flowhelm update status", "Show channel + version status."],
          ["flowhelm update status --json", "JSON output."],
          ["flowhelm update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.flowhelm.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
