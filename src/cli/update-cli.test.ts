...
import type { FlowHelmConfig, ConfigFileSnapshot } from "../config/types.flowhelm.js";
...
vi.mock("../infra/flowhelm-root.js", () => ({
  resolveFlowHelmPackageRoot: vi.fn(),
}));
...
const { resolveFlowHelmPackageRoot } = await import("../infra/flowhelm-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/flowhelm-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as FlowHelmConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/flowhelm-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveFlowHelmPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("flowhelm-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveFlowHelmPackageRoot).mockClear();
    ...
    vi.mocked(resolveFlowHelmPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("flowhelm");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/flowhelm-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "flowhelm-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("FlowHelm update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as FlowHelmConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveFlowHelmPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("flowhelm-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ FLOWHELM_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.FLOWHELM_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("flowhelm-update-wizard");
    await withEnvAsync({ FLOWHELM_GIT_DIR: tempDir }, async () => {
      setTty(true);
