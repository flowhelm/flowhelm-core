...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop flowhelm!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@flowhelm_bot", { botUsername: "flowhelm_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@flowhelm_bot", { botUsername: "flowhelm_bot" })).toBe(true);
