...
import { scheduleGatewaySigusr1Restart, triggerFlowHelmRestart } from "../../infra/restart.js";
...
export const handleSessionCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  return {
    ...
    reply: { text: "⚠️ /session ttl is currently unavailable for non-Discord sessions." },
  };
};
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting FlowHelm in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerFlowHelmRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting FlowHelm via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
