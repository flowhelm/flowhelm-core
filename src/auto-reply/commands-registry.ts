...
export function isCommandEnabled(cfg: FlowHelmConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: FlowHelmConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as FlowHelmConfig),
...
export function getCommandDetection(_cfg?: FlowHelmConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: FlowHelmConfig) {
