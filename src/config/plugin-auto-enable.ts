...
function collectModelRefs(cfg: FlowHelmConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: FlowHelmConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: FlowHelmConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: FlowHelmConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: FlowHelmConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: FlowHelmConfig, pluginId: string): FlowHelmConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
