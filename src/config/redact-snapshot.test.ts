...
import type { ConfigFileSnapshot } from "./types.flowhelm.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.flowhelm/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/flowhelm/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/flowhelm/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/flowhelm/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/flowhelm/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(FlowHelmSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = FlowHelmSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "FlowHelmConfig";
    const hints = mapSensitivePaths(FlowHelmSchema, "", {});
