...
import type { FlowHelmConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: FlowHelmConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: FlowHelmConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = FlowHelmSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as FlowHelmConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as FlowHelmConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as FlowHelmConfig,
  };
}
