...
import type { FlowHelmConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareFlowHelmVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  flowhelm config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  flowhelm config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: FlowHelmConfig,
  pathSegments: string[],
): { changed: boolean; next: FlowHelmConfig } {
...
function coerceConfig(value: unknown): FlowHelmConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as FlowHelmConfig;
}
...
function stampConfigVersion(cfg: FlowHelmConfig): FlowHelmConfig {
...
function warnIfConfigFromFuture(cfg: FlowHelmConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareFlowHelmVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer FlowHelm (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as FlowHelmConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): FlowHelmConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as FlowHelmConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: FlowHelmConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as FlowHelmConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.FLOWHELM_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.FLOWHELM_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.FLOWHELM_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.FLOWHELM_WATCH_SESSION === "string" &&
        deps.env.FLOWHELM_WATCH_SESSION.trim().length > 0
          ? deps.env.FLOWHELM_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.FLOWHELM_WATCH_COMMAND === "string" &&
        deps.env.FLOWHELM_WATCH_COMMAND.trim().length > 0
          ? deps.env.FLOWHELM_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `FLOWHELM_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.FLOWHELM_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.FLOWHELM_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): FlowHelmConfig {
