...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("flowhelm-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "flowhelm-sandbox-browser-global" },
      agentBrowser: { network: "flowhelm-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("flowhelm-sandbox-browser-agent");
  });
