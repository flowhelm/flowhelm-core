...
declare const __FLOWHELM_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "flowhelm";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["FLOWHELM_VERSION"],
      env["FLOWHELM_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current FlowHelm version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __FLOWHELM_VERSION__ === "string" && __FLOWHELM_VERSION__) ||
  process.env.FLOWHELM_BUNDLED_VERSION ||
