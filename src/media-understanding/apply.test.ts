...
const TEMP_MEDIA_PREFIX = "flowhelm-media-";
...
function createGroqAudioConfig(): FlowHelmConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): FlowHelmConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): FlowHelmConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: FlowHelmConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredFlowHelmTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        FLOWHELM_AGENT_DIR: isolatedAgentDir,
