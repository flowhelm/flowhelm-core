  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          flowhelm: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.FLOWHELM_GATEWAY_PORT;
    process.env.FLOWHELM_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.FLOWHELM_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.FLOWHELM_GATEWAY_PASSWORD;
