...
vi.mock("./chrome.js", () => ({
  resolveFlowHelmUserDataDir: vi.fn(() => "/tmp/flowhelm-test/flowhelm/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveFlowHelmUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "flowhelm-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveFlowHelmUserDataDir).mockReturnValue(userDataDir);
