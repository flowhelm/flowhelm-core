...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.FLOWHELM_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/flowhelm" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "flowhelm",
        profiles: {
          flowhelm: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchFlowHelmChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.FLOWHELM_GATEWAY_PORT;
  process.env.FLOWHELM_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.FLOWHELM_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.FLOWHELM_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.FLOWHELM_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.FLOWHELM_GATEWAY_PASSWORD;
  } else {
    process.env.FLOWHELM_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
