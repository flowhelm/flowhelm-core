...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".flowhelm-profile-decorated");
}
 ...
 */
export function decorateFlowHelmProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_FLOWHELM_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_FLOWHELM_BROWSER_COLOR).toUpperCase();
