...
const RELAY_TOKEN_CONTEXT = "flowhelm-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const FLOWHELM_RELAY_BROWSER = "FlowHelm/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.FLOWHELM_GATEWAY_TOKEN?.trim() || process.env.FLOWHELM_BOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or FLOWHELM_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedFlowHelmRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === FLOWHELM_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
