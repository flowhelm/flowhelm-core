import type { BrowserConfig, BrowserProfileConfig, FlowHelmConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_FLOWHELM_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "flowhelm" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_FLOWHELM_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "flowhelm" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_FLOWHELM_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_FLOWHELM_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an FlowHelm browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for flowhelm/flowhelm CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: FlowHelmConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_FLOWHELM_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "flowhelm";
