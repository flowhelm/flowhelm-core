...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredFlowHelmTmpDir: vi.fn(() => "/tmp/flowhelm"),
}));
vi.mock("../infra/tmp-flowhelm-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredFlowHelmTmpDir.mockReturnValue("/tmp/flowhelm");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredFlowHelmTmpDir.mockReturnValue("/tmp/flowhelm-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "flowhelm-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredFlowHelmTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "flowhelm-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "flowhelm-preferred", "downloads")}${path.sep}`),
    );
  });
