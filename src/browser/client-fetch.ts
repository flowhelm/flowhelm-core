...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-flowhelm-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-flowhelm-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-flowhelm-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the FlowHelm gateway (FlowHelm.app menubar, or \`${formatCliCommand("flowhelm gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the FlowHelm browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the FlowHelm browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
