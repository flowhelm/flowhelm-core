...
describe("canvas host state dir defaults", () => {
  it("uses FLOWHELM_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("flowhelm-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("FlowHelm Canvas");
      } finally {
        await handler.close();
      }
    });
