...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.flowhelmCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.flowhelmCanvasA2UIAction &&
        typeof window.flowhelmCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.flowhelmSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("flowhelm:a2ui-action-status", onStatus);
  ...
  function send(name, sourceComponentId) {
    ...
    const sendUserAction =
      typeof window.flowhelmSendUserAction === "function"
        ? window.flowhelmSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FLOWHELM_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.FLOWHELM_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>FlowHelm Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
