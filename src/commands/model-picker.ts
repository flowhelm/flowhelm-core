...
type PromptDefaultModelResult = { model?: string; config?: FlowHelmConfig };
...
function resolveConfiguredModelRaw(cfg: FlowHelmConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: FlowHelmConfig): string[] {
...
export function applyPrimaryModel(cfg: FlowHelmConfig, model: string): FlowHelmConfig {
...
export function applyModelAllowlist(cfg: FlowHelmConfig, models: string[]): FlowHelmConfig {
  const defaults = cfg.agents?.defaults;
