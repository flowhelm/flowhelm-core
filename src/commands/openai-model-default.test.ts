...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: FlowHelmConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: FlowHelmConfig) => { changed: boolean; next: FlowHelmConfig };
  defaultModel: string;
  overrideConfig: FlowHelmConfig;
  alreadyDefaultConfig: FlowHelmConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: FlowHelmConfig) => config,
      applyDefaultConfig: (config: FlowHelmConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: FlowHelmConfig = { agents: { defaults: {} } };
