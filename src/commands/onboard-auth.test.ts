...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under FLOWHELM_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("flowhelm-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-oauth-sync-"));
    ...
    process.env.FLOWHELM_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-oauth-nosync-"));
    process.env.FLOWHELM_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside FLOWHELM_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-oauth-external-"));
    process.env.FLOWHELM_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* FLOWHELM_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FLOWHELM_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("flowhelm-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
