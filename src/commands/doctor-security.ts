...
import type { FlowHelmConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: FlowHelmConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("flowhelm security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.flowhelm/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("flowhelm approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.flowhelm.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("flowhelm configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("flowhelm config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("flowhelm doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("flowhelm config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('flowhelm config set session.dmScope "per-channel-peer"') +
