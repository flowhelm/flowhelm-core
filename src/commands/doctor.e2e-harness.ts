...
export const resolveFlowHelmPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadFlowHelmPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveFlowHelmPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.FLOWHELM_STATE_DIR;
  originalUpdateInProgress = process.env.FLOWHELM_UPDATE_IN_PROGRESS;
  process.env.FLOWHELM_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "flowhelm-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.FLOWHELM_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.FLOWHELM_UPDATE_IN_PROGRESS;
  } else {
    process.env.FLOWHELM_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
