...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".flowhelm", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: FlowHelmConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["FLOWHELM_BOT_GATEWAY_TOKEN", await getenv("FLOWHELM_BOT_GATEWAY_TOKEN")],
    ["FLOWHELM_BOT_GATEWAY_PASSWORD", await getenv("FLOWHELM_BOT_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`FLOWHELM_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["FLOWHELM_GATEWAY_TOKEN", await getenv("FLOWHELM_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["FLOWHELM_GATEWAY_PASSWORD", await getenv("FLOWHELM_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "FLOWHELM_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("FLOWHELM_BOT_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
  ...
  const lines = [
    ...
    "- Use FLOWHELM_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> FLOWHELM_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
