...
const ensureFlowHelmModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/flowhelm-agent/auth.json" });
const resolveFlowHelmAgentDir = vi.fn().mockReturnValue("/tmp/flowhelm-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/flowhelm-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureFlowHelmModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/flowhelm-agent");
  });
