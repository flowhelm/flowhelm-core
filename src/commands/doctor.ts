...
function resolveMode(cfg: FlowHelmConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("FlowHelm doctor");

  const root = await resolveFlowHelmPackageRoot({
  ...
  let cfg: FlowHelmConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("flowhelm configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("flowhelm config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("flowhelm setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("flowhelm doctor --fix")}" to apply changes.`);
  }
