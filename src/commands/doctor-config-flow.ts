...
function stripUnknownConfigKeys(config: FlowHelmConfig): {
  config: FlowHelmConfig;
  removed: string[];
} {
  const parsed = FlowHelmSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: FlowHelmConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: FlowHelmConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: FlowHelmConfig): Promise<{
  config: FlowHelmConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: FlowHelmConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: FlowHelmConfig): {
  config: FlowHelmConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: FlowHelmConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: FlowHelmConfig): {
  config: FlowHelmConfig;
  changes: string[];
} {
...
function collectExecSafeBinScopes(cfg: FlowHelmConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: FlowHelmConfig): ExecSafeBinCoverageHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: FlowHelmConfig): {
  config: FlowHelmConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: FlowHelmConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: FlowHelmConfig): {
  config: FlowHelmConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".flowhelm");
  const targetPath = path.join(targetDir, "flowhelm.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".flowhelm_bot", "flowhelm_bot.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: FlowHelmConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("flowhelm doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("flowhelm doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("flowhelm doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("flowhelm doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("flowhelm doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("flowhelm doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("flowhelm doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "Doctor warnings");
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "flowhelm doctor --fix" to remove these keys.');
    }
  }
