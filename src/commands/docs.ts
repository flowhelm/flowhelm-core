...
const SEARCH_TOOL = "https://docs.flowhelm.ai/mcp.SearchFlowHelm";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.flowhelm.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('flowhelm docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.flowhelm.ai/");
      runtime.log(`Search: ${formatCliCommand('flowhelm docs "your query"')}`);
    }
    return;
  }
