...
export async function statusCommand(
  ...
  runtime.log(theme.heading("FlowHelm status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`flowhelm devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("flowhelm devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("flowhelm devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("flowhelm security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("flowhelm security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.flowhelm.ai/faq");
  runtime.log("Troubleshooting: https://docs.flowhelm.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("flowhelm status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("flowhelm logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("flowhelm status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("flowhelm gateway probe")}`);
  }
}
