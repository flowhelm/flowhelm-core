...
function resolveAgentName(cfg: FlowHelmConfig, agentId: string) {
...
function resolveAgentModel(cfg: FlowHelmConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: FlowHelmConfig): AgentSummary[] {
