...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "FLOWHELM_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("FLOWHELM_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv FLOWHELM_GATEWAY_TOKEN");
    expect(message).not.toContain("FLOWHELM_GATEWAY_PASSWORD");
  });
