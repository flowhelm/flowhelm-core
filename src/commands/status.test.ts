...
beforeAll(() => {
  envSnapshot = captureEnv(["FLOWHELM_PROFILE"]);
  process.env.FLOWHELM_PROFILE = "isolated";
});
...
vi.mock("../infra/flowhelm-root.js", () => ({
  resolveFlowHelmPackageRoot: vi.fn().mockResolvedValue("/tmp/flowhelm"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/flowhelm/pnpm-lock.yaml",
      markerPath: "/tmp/flowhelm/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("FlowHelm status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("flowhelm status --all") ||
          l.includes("flowhelm --profile isolated status --all") ||
          l.includes("flowhelm status --all") ||
          l.includes("flowhelm --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.FLOWHELM_GATEWAY_TOKEN = "abcd1234";
    try {
