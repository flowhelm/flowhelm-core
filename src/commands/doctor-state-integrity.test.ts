...
type EnvSnapshot = {
  HOME?: string;
  FLOWHELM_HOME?: string;
  FLOWHELM_STATE_DIR?: string;
  FLOWHELM_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    FLOWHELM_HOME: process.env.FLOWHELM_HOME,
    FLOWHELM_STATE_DIR: process.env.FLOWHELM_STATE_DIR,
    FLOWHELM_OAUTH_DIR: process.env.FLOWHELM_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: FlowHelmConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: FlowHelmConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "flowhelm-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.FLOWHELM_HOME = tempHome;
    process.env.FLOWHELM_STATE_DIR = path.join(tempHome, ".flowhelm");
    delete process.env.FLOWHELM_OAUTH_DIR;
    fs.mkdirSync(process.env.FLOWHELM_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when FLOWHELM_OAUTH_DIR is explicitly configured", async () => {
    process.env.FLOWHELM_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: FlowHelmConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints flowhelm-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: FlowHelmConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/flowhelm sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/flowhelm sessions cleanup --store ".*sessions\.json" --dry-run/);
