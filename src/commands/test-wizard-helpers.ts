...
export function requireFlowHelmAgentDir(): string {
  const agentDir = process.env.FLOWHELM_AGENT_DIR;
  if (!agentDir) {
    throw new Error("FLOWHELM_AGENT_DIR not set");
  }
  return agentDir;
}
