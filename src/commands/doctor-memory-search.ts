 ...
 * Runs as part of `flowhelm doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("flowhelm config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("flowhelm memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("flowhelm configure --section model")}`,
        `- To disable: ${formatCliCommand("flowhelm config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("flowhelm memory status --deep")}`,
      ].join("\n"),
      "Memory search",
