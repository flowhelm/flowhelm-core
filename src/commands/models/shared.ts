...
export async function loadValidConfigOrThrow(): Promise<FlowHelmConfig> {
...
export async function updateConfig(
  mutator: (cfg: FlowHelmConfig) => FlowHelmConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: FlowHelmConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: FlowHelmConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("flowhelm agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
