...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveFlowHelmAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("flowhelm models auth setup-token")}\` or \`${formatCliCommand("flowhelm configure")}\`.`
          : `Run \`${formatCliCommand("flowhelm configure")}\` or set an API key env var.`;
