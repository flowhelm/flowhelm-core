...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveFlowHelmAgentDir: vi.fn().mockReturnValue("/tmp/flowhelm-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/flowhelm-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/flowhelm-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveFlowHelmAgentDir: mocks.resolveFlowHelmAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/flowhelm-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveFlowHelmAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/flowhelm-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/flowhelm-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/flowhelm-agent-custom");
