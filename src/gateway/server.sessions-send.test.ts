...
const { createFlowHelmTools } = await import("../agents/flowhelm-tools.js");
...
type SessionSendTool = ReturnType<typeof createFlowHelmTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createFlowHelmTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["FLOWHELM_GATEWAY_PORT", "FLOWHELM_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.FLOWHELM_GATEWAY_PORT = String(gatewayPort);
  process.env.FLOWHELM_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      const configPath = process.env.FLOWHELM_CONFIG_PATH;
      if (!configPath) {
        throw new Error("FLOWHELM_CONFIG_PATH missing in gateway test environment");
      }
