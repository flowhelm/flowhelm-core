...
import type { FlowHelmConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: FlowHelmConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: FlowHelmConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: FlowHelmConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: FlowHelmConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: FlowHelmConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: FlowHelmConfig) => {
