...
let lastCreateFlowHelmToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/flowhelm-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateFlowHelmToolsContext?.agentTo,
          agentThreadId: lastCreateFlowHelmToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createFlowHelmTools: (ctx: Record<string, unknown>) => {
      lastCreateFlowHelmToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateFlowHelmToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-flowhelm-message-to": "channel:24514",
        "x-flowhelm-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
