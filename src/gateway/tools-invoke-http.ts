...
import { createFlowHelmTools } from "../agents/flowhelm-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-flowhelm-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-flowhelm-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-flowhelm-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-flowhelm-thread-id")?.trim() || undefined;
  ...
  const allTools = createFlowHelmTools({
