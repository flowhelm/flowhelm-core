...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.FLOWHELM_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.FLOWHELM_BOT_GATEWAY_TOKEN);
}
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.FLOWHELM_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
  ...
  return trimToUndefined(env.FLOWHELM_BOT_GATEWAY_PASSWORD);
}
