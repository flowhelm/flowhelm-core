...
import { resolveFlowHelmAgentDir } from "../agents/agent-paths.js";
...
import { ensureFlowHelmModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, FlowHelmConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.FLOWHELM_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.FLOWHELM_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.FLOWHELM_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<FlowHelmConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.FLOWHELM_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.FLOWHELM_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveFlowHelmAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-live-state-"));
  process.env.FLOWHELM_STATE_DIR = tempStateDir;
  ...
  process.env.FLOWHELM_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.flowhelm-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveFlowHelmAgentDir();
  const sanitizedCfg: FlowHelmConfig = {
    ...params.cfg,
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "FlowHelm live tool probe (local, safe): " +
    ...
    process.env.FLOWHELM_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.FLOWHELM_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureFlowHelmModelsJson(cfg);
      ...
      const rawModels = process.env.FLOWHELM_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.flowhelm-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
