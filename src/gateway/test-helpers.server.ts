...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "FLOWHELM_SKIP_BROWSER_CONTROL_SERVER",
  "FLOWHELM_SKIP_GMAIL_WATCHER",
  "FLOWHELM_SKIP_CANVAS_HOST",
  "FLOWHELM_BUNDLED_PLUGINS_DIR",
  "FLOWHELM_SKIP_CHANNELS",
  "FLOWHELM_SKIP_PROVIDERS",
  "FLOWHELM_SKIP_CRON",
  "FLOWHELM_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.FLOWHELM_STATE_DIR = path.join(tempHome, ".flowhelm");
  delete process.env.FLOWHELM_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.FLOWHELM_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.FLOWHELM_SKIP_GMAIL_WATCHER = "1";
  process.env.FLOWHELM_SKIP_CANVAS_HOST = "1";
  process.env.FLOWHELM_SKIP_CHANNELS = "1";
  process.env.FLOWHELM_SKIP_PROVIDERS = "1";
  process.env.FLOWHELM_SKIP_CRON = "1";
  process.env.FLOWHELM_TEST_MINIMAL_GATEWAY = "1";
  process.env.FLOWHELM_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "flowhelm-test-no-bundled-extensions")
    : "flowhelm-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "flowhelm-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".flowhelm-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__flowhelmTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__flowhelmTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["FLOWHELM_GATEWAY_TOKEN"]);
  const prev = process.env.FLOWHELM_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.FLOWHELM_GATEWAY_TOKEN = fallbackToken;
  }
