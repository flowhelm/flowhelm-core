...
export function resolveHooksConfig(cfg: FlowHelmConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: FlowHelmConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-flowhelm-token"] === "string"
      ? req.headers["x-flowhelm-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
