...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("flowhelm-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("flowhelm-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("flowhelm-lid-a-", (first) => {
      withTempDirSync("flowhelm-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.flowhelm when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "flowhelm-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers FLOWHELM_HOME over HOME", () => {
    vi.stubEnv("FLOWHELM_HOME", "/srv/flowhelm-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/flowhelm-home"));
...
describe("shortenHomePath", () => {
  it("uses $FLOWHELM_HOME prefix when FLOWHELM_HOME is set", () => {
    vi.stubEnv("FLOWHELM_HOME", "/srv/flowhelm-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/flowhelm-home")}/.flowhelm/flowhelm.json`)).toBe(
      "$FLOWHELM_HOME/.flowhelm/flowhelm.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $FLOWHELM_HOME replacement when FLOWHELM_HOME is set", () => {
    vi.stubEnv("FLOWHELM_HOME", "/srv/flowhelm-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/flowhelm-home")}/.flowhelm/flowhelm.json`),
    ).toBe("config: $FLOWHELM_HOME/.flowhelm/flowhelm.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/flowhelm")).toBe(path.resolve(os.homedir(), "flowhelm"));
  });
  ...
  it("prefers FLOWHELM_HOME for tilde expansion", () => {
    vi.stubEnv("FLOWHELM_HOME", "/srv/flowhelm-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/flowhelm")).toBe(path.resolve("/srv/flowhelm-home", "flowhelm"));
