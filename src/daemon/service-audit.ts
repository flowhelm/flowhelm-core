...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.FLOWHELM_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service FLOWHELM_GATEWAY_TOKEN does not match gateway.auth.token in flowhelm.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `flowhelm gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
