...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses FLOWHELM_HOME for tilde expansion", () => {
    vi.stubEnv("FLOWHELM_HOME", "/srv/flowhelm-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/flowhelm-home", "cron", "jobs.json"));
  });
});
