...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".flowhelm");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.FLOWHELM_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$FLOWHELM_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via FLOWHELM_STATE_DIR.
