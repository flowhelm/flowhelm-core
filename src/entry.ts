...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "flowhelm.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "flowhelm.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.FLOWHELM_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.FLOWHELM_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.FLOWHELM_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[flowhelm] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[flowhelm] ${parsed.error}`);
      process.exit(2);
    }
