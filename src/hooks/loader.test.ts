...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["FLOWHELM_BUNDLED_HOOKS_DIR"]);
    process.env.FLOWHELM_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
