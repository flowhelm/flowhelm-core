...
This directory contains hooks that ship with FlowHelm. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.flowhelm/flowhelm.json`:
...
2. Restart gateway: `pkill -9 -f 'flowhelm.*gateway' && pnpm flowhelm gateway`
3. Enable the hook: `flowhelm hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.flowhelm.ai/automation/hooks
