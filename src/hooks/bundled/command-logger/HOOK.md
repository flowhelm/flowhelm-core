...
homepage: https://docs.flowhelm.ai/automation/hooks#command-logger
metadata:
  {
    "flowhelm":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.flowhelm/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.flowhelm/logs/commands.log ~/.flowhelm/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/flowhelm`:
   ```
   /home/username/.flowhelm/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
