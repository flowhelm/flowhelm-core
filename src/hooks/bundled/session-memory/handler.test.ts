...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => FlowHelmConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("flowhelm-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): FlowHelmConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
