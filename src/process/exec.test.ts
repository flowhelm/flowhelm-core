...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ FLOWHELM_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.FLOWHELM_BASE_ENV ?? "") + "|" + (process.env.FLOWHELM_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { FLOWHELM_TEST_ENV: "ok" },
        },
      );
