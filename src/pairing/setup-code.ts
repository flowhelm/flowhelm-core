...
function resolveGatewayPort(cfg: FlowHelmConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: FlowHelmConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    env.FLOWHELM_GATEWAY_TOKEN?.trim() ||
    env.FLOWHELM_BOT_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    env.FLOWHELM_GATEWAY_PASSWORD?.trim() ||
    env.FLOWHELM_BOT_GATEWAY_PASSWORD?.trim() ||
