...
export const DEFAULT_LOG_DIR = resolvePreferredFlowHelmTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "flowhelm.log"); // legacy single-file path

const LOG_PREFIX = "flowhelm";
...
function resolveSettings(): ResolvedSettings {
  let cfg: FlowHelmConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => FlowHelmConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.FLOWHELM_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[flowhelm] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
