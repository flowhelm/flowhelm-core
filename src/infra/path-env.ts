...
function candidateBinDirs(opts: EnsureFlowHelmPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `flowhelm` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "flowhelm");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.FLOWHELM_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "flowhelm"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `flowhelm` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureFlowHelmCliOnPath(opts: EnsureFlowHelmPathOpts = {}) {
  if (isTruthyEnvValue(process.env.FLOWHELM_PATH_BOOTSTRAPPED)) {
    return;
  }
