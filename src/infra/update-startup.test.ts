...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["FLOWHELM_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.FLOWHELM_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveFlowHelmPackageRoot } = await import("./flowhelm-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveFlowHelmPackageRoot).mockResolvedValue("/opt/flowhelm");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/flowhelm/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
