...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.FLOWHELM_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.FLOWHELM_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set FLOWHELM_APNS_TEAM_ID and FLOWHELM_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.FLOWHELM_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.FLOWHELM_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.FLOWHELM_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set FLOWHELM_APNS_PRIVATE_KEY_P8 or FLOWHELM_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading FLOWHELM_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createFlowHelmPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    flowhelm: createFlowHelmPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
