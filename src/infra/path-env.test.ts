...
let ensureFlowHelmCliOnPath: typeof import("./path-env.js").ensureFlowHelmCliOnPath;

describe("ensureFlowHelmCliOnPath", () => {
  const envKeys = [
    "PATH",
    "FLOWHELM_PATH_BOOTSTRAPPED",
    "FLOWHELM_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureFlowHelmCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling flowhelm exists", () => {
    const tmp = abs("/tmp/flowhelm-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "flowhelm");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.FLOWHELM_PATH_BOOTSTRAPPED;

    ensureFlowHelmCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.FLOWHELM_PATH_BOOTSTRAPPED = "1";
    ensureFlowHelmCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/flowhelm-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "flowhelm");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/flowhelm-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "flowhelm");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "flowhelm");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/flowhelm-path/case-linuxbrew");
