...
vi.mock("./flowhelm-root.js", () => ({
  resolveFlowHelmPackageRoot: vi.fn(async () => null),
  resolveFlowHelmPackageRootSync: vi.fn(() => null),
}));
...
let flowhelmRoot: typeof import("./flowhelm-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    flowhelmRoot = await import("./flowhelm-root.js");
  });
  ...
  it("uses resolveFlowHelmPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/flowhelm");
    (
      flowhelmRoot.resolveFlowHelmPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/flowhelm"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "flowhelm" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "flowhelm.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-flowhelm");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/flowhelm-bundle");
    (
      flowhelmRoot.resolveFlowHelmPackageRootSync as unknown as ReturnType<typeof vi.fn>
