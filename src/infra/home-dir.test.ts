...
describe("resolveEffectiveHomeDir", () => {
  it("prefers FLOWHELM_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/flowhelm-home"),
    );
  });
  ...
  it("expands FLOWHELM_HOME when set to ~", () => {
    const env = {
      FLOWHELM_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for FLOWHELM_HOME", () => {
    const result = resolveRequiredHomeDir(
      { FLOWHELM_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when FLOWHELM_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ FLOWHELM_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { FLOWHELM_HOME: "/srv/flowhelm-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/flowhelm-home")}/x`);
  });
