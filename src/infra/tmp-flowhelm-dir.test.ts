...
import { POSIX_FLOWHELM_TMP_DIR, resolvePreferredFlowHelmTmpDir } from "./tmp-flowhelm-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredFlowHelmTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `flowhelm-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredFlowHelmTmpDir({
    accessSync,
...
describe("resolvePreferredFlowHelmTmpDir", () => {
  it("prefers /tmp/flowhelm when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_FLOWHELM_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/flowhelm when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_FLOWHELM_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/flowhelm when /tmp/flowhelm is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/flowhelm when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/flowhelm is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/flowhelm is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/flowhelm is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
