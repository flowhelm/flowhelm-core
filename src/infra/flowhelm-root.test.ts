...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__flowhelm_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "flowhelm-root");
...
describe("resolveFlowHelmPackageRoot", () => {
  let resolveFlowHelmPackageRoot: typeof import("./flowhelm-root.js").resolveFlowHelmPackageRoot;
  let resolveFlowHelmPackageRootSync: typeof import("./flowhelm-root.js").resolveFlowHelmPackageRootSync;

  beforeAll(async () => {
    ({ resolveFlowHelmPackageRoot, resolveFlowHelmPackageRootSync } =
      await import("./flowhelm-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "flowhelm");
    const pkgRoot = path.join(project, "node_modules", "flowhelm");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "flowhelm" }));

    expect(resolveFlowHelmPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "flowhelm");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "flowhelm.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "flowhelm" }));

    expect(resolveFlowHelmPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveFlowHelmPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-flowhelm package roots", async () => {
    const pkgRoot = fx("not-flowhelm");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-flowhelm" }));

    expect(resolveFlowHelmPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveFlowHelmPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveFlowHelmPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
