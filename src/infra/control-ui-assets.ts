...
import { resolveFlowHelmPackageRoot, resolveFlowHelmPackageRootSync } from "./flowhelm-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveFlowHelmPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "flowhelm" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveFlowHelmPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // flowhelm.mjs or dist/<bundle>.js
