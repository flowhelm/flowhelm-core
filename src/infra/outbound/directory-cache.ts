...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: FlowHelmConfig | null = null;
  ...
  get(key: string, cfg: FlowHelmConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: FlowHelmConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: FlowHelmConfig): void {
