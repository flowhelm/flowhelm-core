...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: FlowHelmConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: FlowHelmConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: FlowHelmConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: FlowHelmConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: FlowHelmConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: FlowHelmConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: FlowHelmConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
