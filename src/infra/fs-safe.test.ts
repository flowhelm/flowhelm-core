...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("flowhelm-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await tempDirs.make("flowhelm-fs-safe-root-");
    const outside = await tempDirs.make("flowhelm-fs-safe-outside-");
