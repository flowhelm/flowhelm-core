...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.FLOWHELM_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*flowhelm-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "flowhelm";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "flowhelm");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "flowhelm-gw");
  ...
  records.push(`_flowhelm-gw._tcp IN PTR ${instanceLabel}._flowhelm-gw._tcp`);
  records.push(`${instanceLabel}._flowhelm-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._flowhelm-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; flowhelm-content-hash: ${contentHash}\n${contentBody}`;
}
