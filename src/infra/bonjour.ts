...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FLOWHELM_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "FlowHelm";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(FlowHelm\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.FLOWHELM_MDNS_HOSTNAME?.trim() ||
    process.env.FLOWHELM_BOT_MDNS_HOSTNAME?.trim() ||
    "flowhelm";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
