...
const DEFAULT_PACKAGE_NAME = "flowhelm";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "flowhelm-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "flowhelm.mjs");
    ...
    const doctorStep = await runStep(
      step("flowhelm doctor", doctorArgv, gitRoot, { FLOWHELM_UPDATE_IN_PROGRESS: "1" }),
    );
