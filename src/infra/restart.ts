...
export function triggerFlowHelmRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.FLOWHELM_SYSTEMD_UNIT,
        process.env.FLOWHELM_PROFILE,
      );
  ...
  const label =
    process.env.FLOWHELM_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.FLOWHELM_PROFILE);
