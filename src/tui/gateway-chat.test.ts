...
describe("resolveGatewayConnection", () => {
  ...
  it("uses FLOWHELM_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ FLOWHELM_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers FLOWHELM_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ FLOWHELM_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
