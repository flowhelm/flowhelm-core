...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: FlowHelmConfig, policy: ChannelAccessPolicy) => FlowHelmConfig;
  resolveAllowlist: (params: { cfg: FlowHelmConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: FlowHelmConfig; resolved: TResolved }) => FlowHelmConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: FlowHelmConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: FlowHelmConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: FlowHelmConfig, policy: ChannelAccessPolicy): FlowHelmConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: FlowHelmConfig, policy: ChannelAccessPolicy): FlowHelmConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: FlowHelmConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: FlowHelmConfig; resolved: string[] }) => {
      calls.push("apply");
