...
import { discoverFlowHelmPlugins } from "../../plugins/discovery.js";
import type { FlowHelmPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, FlowHelmPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["FLOWHELM_PLUGIN_CATALOG_PATHS", "FLOWHELM_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<FlowHelmPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: FlowHelmPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: FlowHelmPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverFlowHelmPlugins({ workspaceDir: options.workspaceDir });
