...
function cfg(accounts?: Record<string, unknown> | null): FlowHelmConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as FlowHelmConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as FlowHelmConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as FlowHelmConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as FlowHelmConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as FlowHelmConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as FlowHelmConfig)).toBe("default");
    });
  });
});
