...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    flowhelmHome: process.env.FLOWHELM_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("FLOWHELM_HOME", snapshot.flowhelmHome);
  restoreKey("FLOWHELM_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked FLOWHELM_HOME.
  delete process.env.FLOWHELM_HOME;
  process.env.FLOWHELM_STATE_DIR = path.join(base, ".flowhelm");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "flowhelm-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".flowhelm", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
