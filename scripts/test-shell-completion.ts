 ...
 * `flowhelm update`. Use it to verify the completion installation flow
...
\`flowhelm update\`, \`flowhelm doctor\`, and \`flowhelm onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# FlowHelm Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`flowhelm completion --install\` later to enable.`));
    return;
  }
