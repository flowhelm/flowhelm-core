#!/bin/bash
# Setup FlowHelm Auth Management System
...
echo "=== FlowHelm Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'flowhelm-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  FlowHelm message: Send warning via FlowHelm itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/flowhelm-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/flowhelm-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/flowhelm-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now flowhelm-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status flowhelm-auth-monitor.timer"
echo ""
echo "Quick commands:"
