...
const shouldBuild = (deps) => {
  if (deps.env.FLOWHELM_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.FLOWHELM_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[flowhelm] ${message}\n`);
};

const runFlowHelm = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["flowhelm.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
