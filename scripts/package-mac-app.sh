...
# Build and bundle FlowHelm into a minimal .app we can open.
# Outputs to dist/FlowHelm.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/FlowHelm.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="FlowHelm"
BUNDLE_ID="${BUNDLE_ID:-ai.flowhelm.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/flowhelm/flowhelm/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/FlowHelm/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :FlowHelmBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :FlowHelmGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/FlowHelm"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/FlowHelm"
fi
chmod +x "$APP_ROOT/Contents/MacOS/FlowHelm"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/FlowHelm" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/FlowHelm/Resources/FlowHelm.icns" "$APP_ROOT/Contents/Resources/FlowHelm.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/FlowHelm/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying FlowHelmKit resources"
FLOWHELMKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/FlowHelmKit_FlowHelmKit.bundle"
if [ -d "$FLOWHELMKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/FlowHelmKit_FlowHelmKit.bundle"
  cp -R "$FLOWHELMKIT_BUNDLE" "$APP_ROOT/Contents/Resources/FlowHelmKit_FlowHelmKit.bundle"
else
  echo "WARN: FlowHelmKit resource bundle not found at $FLOWHELMKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running FlowHelm"
killall -q FlowHelm 2>/dev/null || true
