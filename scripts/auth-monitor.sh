...
#
# Suggested cron: */30 * * * * /home/admin/flowhelm/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send FlowHelm notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., flowhelm-alerts)
...
STATE_FILE="$HOME/.flowhelm/auth-monitor-state"
...
send_notification() {
    ...
    # Send via FlowHelm if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use flowhelm
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via FlowHelm to $NOTIFY_PHONE..."
            flowhelm send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! FlowHelm is down. Run: ssh l36 '~/flowhelm/scripts/mobile-reauth.sh'" "urgent"
    exit 1
