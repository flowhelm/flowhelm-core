...
# FlowHelm Auth Widget for Termux
...
# Check auth status
termux-toast "Checking FlowHelm auth..."

STATUS=$(ssh "$SERVER" '$HOME/flowhelm/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/flowhelm/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|FLOWHELM_EXPIRING|FLOWHELM_BOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/flowhelm/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|FLOWHELM_EXPIRED|FLOWHELM_BOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: FlowHelm won't work until re-auth"
                ;;
        esac
        ;;
