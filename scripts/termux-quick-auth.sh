...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/flowhelm/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "FlowHelm Re-Auth" -c "After getting key, run: ssh $SERVER '~/flowhelm/scripts/mobile-reauth.sh'" --id flowhelm-auth
        ;;
    *)
