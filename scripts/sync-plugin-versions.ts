...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core FlowHelm release numbers.\n\n`;
...
function stripWorkspaceFlowHelmDevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.flowhelm !== "workspace:*") {
    return false;
  }
  delete devDeps.flowhelm;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceFlowHelmDevDependency(pkg);
