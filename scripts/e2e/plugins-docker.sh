...
IMAGE_NAME="flowhelm-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    FLOWHELM_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    FLOWHELM_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export FLOWHELM_ENTRY

	  home_dir=$(mktemp -d "/tmp/flowhelm-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.flowhelm/extensions/demo-plugin"

  cat > "$HOME/.flowhelm/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.flowhelm/extensions/demo-plugin/flowhelm.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$FLOWHELM_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/flowhelm-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@flowhelm/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/flowhelm.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$FLOWHELM_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$FLOWHELM_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/flowhelm-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@flowhelm/demo-plugin-dir",
  "version": "0.0.1",
  "flowhelm": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/flowhelm.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$FLOWHELM_ENTRY" plugins install "$dir_plugin"
	  node "$FLOWHELM_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/flowhelm-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@flowhelm/demo-plugin-file",
  "version": "0.0.1",
  "flowhelm": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/flowhelm.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$FLOWHELM_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$FLOWHELM_ENTRY" plugins list --json > /tmp/plugins4.json
