...
IMAGE_NAME="flowhelm-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="flowhelm-net-e2e-$$"
GW_NAME="flowhelm-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "FLOWHELM_GATEWAY_TOKEN=$TOKEN" \
	  -e "FLOWHELM_SKIP_CHANNELS=1" \
	  -e "FLOWHELM_SKIP_GMAIL_WATCHER=1" \
	  -e "FLOWHELM_SKIP_CRON=1" \
	  -e "FLOWHELM_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
