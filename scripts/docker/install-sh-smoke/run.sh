...
INSTALL_URL="${FLOWHELM_INSTALL_URL:-https://flowhelm.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${FLOWHELM_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${FLOWHELM_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="flowhelm"
PACKAGE_NAME="${FLOWHELM_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (FLOWHELM_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${FLOWHELM_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${FLOWHELM_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
