#!/usr/bin/env bash
# ClawDock - Docker helpers for FlowHelm
# Inspired by Simon Willison's "Running FlowHelm in Docker"
# https://til.simonwillison.net/llms/flowhelm-docker
#
# Installation:
...
# Common paths to check for FlowHelm
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/flowhelm"
  "${HOME}/workspace/flowhelm"
  "${HOME}/projects/flowhelm"
  "${HOME}/dev/flowhelm"
  "${HOME}/code/flowhelm"
  "${HOME}/src/flowhelm"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found FlowHelm at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/flowhelm"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ FlowHelm not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/flowhelm/flowhelm.git ~/flowhelm"
    echo "  cd ~/flowhelm && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^FLOWHELM_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d flowhelm-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart flowhelm-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f flowhelm-gateway
}
...
clawdock-workspace() {
  cd ~/.flowhelm/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec flowhelm-gateway \
    bash -c 'echo "alias flowhelm=\"./flowhelm.mjs\"" > /tmp/.bashrc_flowhelm && bash --rcfile /tmp/.bashrc_flowhelm'
}
...
clawdock-cli() {
  _clawdock_compose run --rm flowhelm-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build flowhelm-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "FLOWHELM_GATEWAY_TOKEN=$token" flowhelm-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" flowhelm-gateway \
    bash -c './flowhelm.mjs config set gateway.remote.token "$TOKEN" && ./flowhelm.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec flowhelm-gateway \
    bash -c "./flowhelm.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart flowhelm-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm flowhelm-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec flowhelm-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'flowhelm config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for FlowHelm${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (flowhelm alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to flowhelm project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.flowhelm)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'flowhelm channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'flowhelm status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.flowhelm.ai${_CLR_RESET}"
  echo ""
}
