...
BASE_IMAGE="${BASE_IMAGE:-flowhelm-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-flowhelm-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=flowhelm.sandbox=1)
NOTE
