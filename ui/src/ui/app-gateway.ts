...
export function connectGateway(host: GatewayHost) {
  ...
  const client = new GatewayBrowserClient({
    ...
    clientName: "flowhelm-control-ui",
    mode: "webchat",
    ...
    onHello: (hello) => {
      ...
      void loadAssistantIdentity(host as unknown as FlowHelmApp);
      void loadAgents(host as unknown as FlowHelmApp);
      void loadToolsCatalog(host as unknown as FlowHelmApp);
      void loadNodes(host as unknown as FlowHelmApp, { quiet: true });
      void loadDevices(host as unknown as FlowHelmApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (state === "final") {
    void loadSessions(host as unknown as FlowHelmApp, {
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as FlowHelmApp, payload);
  handleTerminalChatEvent(host, payload, state);
  if (state === "final" && shouldReloadHistoryForFinalEvent(payload)) {
    void loadChatHistory(host as unknown as FlowHelmApp);
  }
}
