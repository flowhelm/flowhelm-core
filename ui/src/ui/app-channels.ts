...
export async function handleWhatsAppStart(host: FlowHelmApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: FlowHelmApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: FlowHelmApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: FlowHelmApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: FlowHelmApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: FlowHelmApp): string {
...
function resolveGatewayHttpAuthHeader(host: FlowHelmApp): string | null {
...
function buildGatewayHttpHeaders(host: FlowHelmApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: FlowHelmApp) {
...
export function handleNostrProfileToggleAdvanced(host: FlowHelmApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: FlowHelmApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: FlowHelmApp) {
  const state = host.nostrProfileFormState;
