...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/flowhelm/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/flowhelm");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/flowhelm/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__FLOWHELM_CONTROL_UI_BASE_PATH__ = "/flowhelm";
    const app = mountApp("/flowhelm/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/flowhelm");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/flowhelm/sessions");
  });
