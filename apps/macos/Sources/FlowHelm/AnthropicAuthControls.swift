...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: FlowHelmOAuthStore.AnthropicOAuthStatus = FlowHelmOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: FlowHelmOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = FlowHelmOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = FlowHelmOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try FlowHelmOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. FlowHelm can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: FlowHelmOAuthStore.AnthropicOAuthStatus,
