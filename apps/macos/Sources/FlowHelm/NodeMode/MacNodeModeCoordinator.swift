...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.flowhelm", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: FlowHelmNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [FlowHelmCapability.canvas.rawValue, FlowHelmCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(FlowHelmCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if FlowHelmLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(FlowHelmCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            FlowHelmSystemCommand.which.rawValue,
            FlowHelmSystemCommand.run.rawValue,
            FlowHelmSystemCommand.execApprovalsGet.rawValue,
            FlowHelmSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(FlowHelmCapability.camera.rawValue) {
        ...
        if capsSet.contains(FlowHelmCapability.location.rawValue) {
