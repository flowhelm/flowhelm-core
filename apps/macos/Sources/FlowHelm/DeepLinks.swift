...
private let deepLinkLogger = Logger(subsystem: "ai.flowhelm", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "FlowHelm is paused", message: "Unpause FlowHelm to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run FlowHelm agent?", message: body) else { return }
        }
