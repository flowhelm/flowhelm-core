...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.flowhelm.mac`) even if the shipped
    ...
    {
        if environment["FLOWHELM_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "flowhelm.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "flowhelm.nixMode") { return true }
