...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["FLOWHELM_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/flowhelm")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("flowhelm-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("flowhelm-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/flowhelm/ (rolling or stdout), or nil if none exist.
