...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: FlowHelmProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.flowhelm", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: FlowHelmKit.AnyCodable]()) {
                $0[$1.key] = FlowHelmKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: FlowHelmProtocol.AnyCodable?) -> [String: FlowHelmProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: FlowHelmProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: FlowHelmKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: FlowHelmProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("flowhelm.control.heartbeat")
    static let controlAgentEvent = Notification.Name("flowhelm.control.agent")
}
