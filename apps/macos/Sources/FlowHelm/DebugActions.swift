...
enum DebugActions {
    private static let verboseDefaultsKey = "flowhelm.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "flowhelm.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = FlowHelmPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "FlowHelm", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FlowHelmPaths.configURL
        guard
