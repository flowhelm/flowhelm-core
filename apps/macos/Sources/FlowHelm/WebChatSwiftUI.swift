...
private let webChatSwiftLogger = Logger(subsystem: "ai.flowhelm", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: FlowHelmChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> FlowHelmChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(FlowHelmChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> FlowHelmChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                FlowHelmGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    FlowHelmAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<FlowHelmChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any FlowHelmChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = FlowHelmChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: FlowHelmChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<FlowHelmChatView>) -> NSViewController
    {
