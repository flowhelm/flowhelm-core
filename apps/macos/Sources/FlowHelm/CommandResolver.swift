...
enum CommandResolver {
    private static let projectRootDefaultsKey = "flowhelm.gatewayProjectRootPath"
    private static let helperName = "flowhelm"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let flowhelmEntry = root.appendingPathComponent("flowhelm.mjs").path
        if FileManager().isReadableFile(atPath: flowhelmEntry) { return flowhelmEntry }
        let binEntry = root.appendingPathComponent("bin/flowhelm.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/flowhelm")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let flowhelmPaths = self.flowhelmManagedPaths(home: home)
        if !flowhelmPaths.isEmpty {
            extras.insert(contentsOf: flowhelmPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + flowhelmPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func flowhelmManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".flowhelm"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func flowhelmExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectFlowHelmExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("flowhelm.mjs").path,
            root.appendingPathComponent("bin/flowhelm.js").path,
        ]
    ...
    static func hasAnyFlowHelmInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.flowhelmExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func flowhelmNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let flowhelmPath = self.projectFlowHelmExecutable(projectRoot: root) {
                return [flowhelmPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "flowhelm", subcommand] + extraArgs
            }
            if let flowhelmPath = self.flowhelmExecutable(searchPaths: searchPaths) {
                return [flowhelmPath, subcommand] + extraArgs
            }

            let missingEntry = """
            flowhelm entrypoint missing (looked for dist/index.js or flowhelm.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real flowhelm CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/flowhelm"
        ...
        \(cliSection)
        \(projectSection)
        if command -v flowhelm >/dev/null 2>&1; then
          CLI="$(command -v flowhelm)"
          flowhelm \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/flowhelm.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/flowhelm.mjs"
            node "$PRJ/flowhelm.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/flowhelm.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/flowhelm.js"
            node "$PRJ/bin/flowhelm.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent flowhelm"
          pnpm --silent flowhelm \(quotedArgs);
        else
          echo "flowhelm CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? FlowHelmConfigFile.loadDict()
