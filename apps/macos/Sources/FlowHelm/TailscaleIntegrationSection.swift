...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.flowhelm/flowhelm.json. Prefer FLOWHELM_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.flowhelm/flowhelm.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.flowhelm/flowhelm.json. Restart the gateway to apply."
        }
