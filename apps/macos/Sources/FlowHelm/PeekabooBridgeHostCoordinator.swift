...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.flowhelm", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: FlowHelmPeekabooBridgeServices?
    private static var flowhelmSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("FlowHelm", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = FlowHelmPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.flowhelmSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.flowhelmSocketPath, privacy: .public)")
    }
...
@MainActor
private final class FlowHelmPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.flowhelm.peekaboo")
