...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.flowhelm", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> FlowHelmSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: FlowHelmSessionPreviewEntry,
    ...
    private static func cache(payload: FlowHelmSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: FlowHelmChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [FlowHelmKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [FlowHelmKit.AnyCodable]) -> [FlowHelmChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(FlowHelmChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: FlowHelmChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: FlowHelmChatMessage) -> Bool {
    ...
    private static func toolNames(for message: FlowHelmChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: FlowHelmChatMessage) -> String? {
