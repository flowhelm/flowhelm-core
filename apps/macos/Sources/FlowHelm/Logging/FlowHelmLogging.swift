...
enum FlowHelmLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = FlowHelmOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = FlowHelmFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        FlowHelmLogging.bootstrapIfNeeded()
        let label = FlowHelmLogging.makeLabel(subsystem: subsystem, category: category)
...
struct FlowHelmOSLogHandler: LogHandler {
...
struct FlowHelmFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = FlowHelmLogging.parseLabel(self.label)
