...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: FlowHelmProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: FlowHelmProtocol.AnyCodable] = [
            "presence": .init([FlowHelmProtocol.AnyCodable(entry)]),
        ]
        let payload = FlowHelmProtocol.AnyCodable(payloadMap)
