...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["flowhelm"]
        let text = "hey flowhelm"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey flowhelm do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["flowhelm"], minPostTriggerGap: 0.3)
