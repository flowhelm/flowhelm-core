...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: FlowHelmProtocol.AnyCodable(["ok": FlowHelmProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: FlowHelmProtocol.AnyCodable(["ok": FlowHelmProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = FlowHelmProtocol.AnyCodable([
            "runId": FlowHelmProtocol.AnyCodable("run-1"),
            "sessionKey": FlowHelmProtocol.AnyCodable("main"),
            "state": FlowHelmProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: FlowHelmProtocol.AnyCodable(["a": FlowHelmProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
