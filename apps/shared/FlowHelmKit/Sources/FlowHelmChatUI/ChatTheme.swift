...
enum FlowHelmChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FlowHelmChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FlowHelmChatTheme.onboardingAssistantBubble"),
...
enum FlowHelmPlatformImageFactory {
    static func image(_ image: FlowHelmPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
