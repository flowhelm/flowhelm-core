...
private let chatUILogger = Logger(subsystem: "ai.flowhelm", category: "FlowHelmChatUI")

@MainActor
@Observable
public final class FlowHelmChatViewModel {
    public private(set) var messages: [FlowHelmChatMessage] = []
    ...
    public var attachments: [FlowHelmPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [FlowHelmChatPendingToolCall] = []
    public private(set) var sessions: [FlowHelmChatSessionEntry] = []
    private let transport: any FlowHelmChatTransport
    ...
    private var pendingToolCallsById: [String: FlowHelmChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any FlowHelmChatTransport) {
    ...
    public var sessionChoices: [FlowHelmChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [FlowHelmChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: FlowHelmPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [FlowHelmChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: FlowHelmChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: FlowHelmChatMessage) -> FlowHelmChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> FlowHelmChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return FlowHelmChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: FlowHelmChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [FlowHelmChatMessage],
        incoming: [FlowHelmChatMessage]) -> [FlowHelmChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [FlowHelmChatMessage]) -> [FlowHelmChatMessage] {
        var result: [FlowHelmChatMessage] = []
    ...
    private static func dedupeKey(for message: FlowHelmChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [FlowHelmChatMessageContent] = [
            FlowHelmChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> FlowHelmChatAttachmentPayload in
            FlowHelmChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> FlowHelmChatSessionEntry {
        FlowHelmChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: FlowHelmChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: FlowHelmChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: FlowHelmAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = FlowHelmChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> FlowHelmPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
