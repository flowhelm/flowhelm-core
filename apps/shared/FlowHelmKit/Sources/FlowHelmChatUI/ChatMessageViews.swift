...
struct ChatMessageBubble: View {
    let message: FlowHelmChatMessage
    let style: FlowHelmChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? FlowHelmChatTheme.userText : FlowHelmChatTheme.assistantText
    ...
    private var inlineAttachments: [FlowHelmChatMessageContent] {
    ...
    private var toolCalls: [FlowHelmChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [FlowHelmChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? FlowHelmChatTheme.userBubble
        }
        if self.style == .onboarding {
            return FlowHelmChatTheme.onboardingAssistantBubble
        }
        return FlowHelmChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return FlowHelmChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: FlowHelmChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? FlowHelmChatTheme.userText : FlowHelmChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: FlowHelmChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(FlowHelmChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(FlowHelmChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [FlowHelmChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: FlowHelmChatTheme.assistantText)
            }
        }
    }
}
