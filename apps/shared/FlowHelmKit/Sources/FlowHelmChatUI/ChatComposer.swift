...
@MainActor
struct FlowHelmChatComposer: View {
    @Bindable var viewModel: FlowHelmChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(FlowHelmChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(FlowHelmChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \FlowHelmPendingAttachment.id)
                { (att: FlowHelmPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            FlowHelmPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(FlowHelmChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(FlowHelmChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(FlowHelmChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(FlowHelmChatTheme.subtleCard)
        .clipShape(Capsule())
    }
