...
import Foundation
import Testing
@testable import FlowHelmChatUI
...
private final class TestChatTransport: @unchecked Sendable, FlowHelmChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [FlowHelmChatHistoryPayload]
    private let sessionsResponses: [FlowHelmChatSessionsListResponse]

    private let stream: AsyncStream<FlowHelmChatTransportEvent>
    private let continuation: AsyncStream<FlowHelmChatTransportEvent>.Continuation

    init(
        historyResponses: [FlowHelmChatHistoryPayload],
        sessionsResponses: [FlowHelmChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<FlowHelmChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> FlowHelmChatHistoryPayload {
        ...
        return self.historyResponses.last ?? FlowHelmChatHistoryPayload(
        ...
        attachments _: [FlowHelmChatAttachmentPayload]) async throws -> FlowHelmChatSendResponse
    {
        ...
        return FlowHelmChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> FlowHelmChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? FlowHelmChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: FlowHelmChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = FlowHelmChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = FlowHelmChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { FlowHelmChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = FlowHelmChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = FlowHelmChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = FlowHelmChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { FlowHelmChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
