...
final class RemindersService: RemindersServicing {
    func list(params: FlowHelmRemindersListParams) async throws -> FlowHelmRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[FlowHelmReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return FlowHelmReminderPayload(
        ...
        return FlowHelmRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: FlowHelmRemindersAddParams) async throws -> FlowHelmRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = FlowHelmReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return FlowHelmRemindersAddPayload(reminder: payload)
    }
