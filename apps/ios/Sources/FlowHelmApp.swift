...
@MainActor
final class FlowHelmAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.flowhelm.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.flowhelm.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.flowhelm.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "flowhelm.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "flowhelm.watch.promptId"
    static let sessionKeyKey = "flowhelm.watch.sessionKey"
    static let actionPrimaryIDKey = "flowhelm.watch.action.primary.id"
    static let actionPrimaryLabelKey = "flowhelm.watch.action.primary.label"
    static let actionSecondaryIDKey = "flowhelm.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "flowhelm.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "flowhelm.watch.action.primary"
    static let actionSecondaryIdentifier = "flowhelm.watch.action.secondary"
    static let actionIdentifierPrefix = "flowhelm.watch.action."
    static let actionIDKeyPrefix = "flowhelm.watch.action.id."
    static let actionLabelKeyPrefix = "flowhelm.watch.action.label."
    static let categoryPrefix = "flowhelm.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: FlowHelmWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> FlowHelmWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return FlowHelmWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "FlowHelm" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [FlowHelmWatchAction]) -> [UNNotificationAction] {
...
@main
struct FlowHelmApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(FlowHelmAppDelegate.self) private var appDelegate
...
extension FlowHelmApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("FlowHelm: installing uncaught exception handler")
