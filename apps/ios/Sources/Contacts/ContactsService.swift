...
final class ContactsService: ContactsServicing {
    ...
    func search(params: FlowHelmContactsSearchParams) async throws -> FlowHelmContactsSearchPayload {
        ...
        return FlowHelmContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: FlowHelmContactsAddParams) async throws -> FlowHelmContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return FlowHelmContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return FlowHelmContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> FlowHelmContactPayload {
        FlowHelmContactPayload(
