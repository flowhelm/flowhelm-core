...
final class MotionService: MotionServicing {
    func activities(params: FlowHelmMotionActivityParams) async throws -> FlowHelmMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[FlowHelmMotionActivityEntry], Error>) in
        ...
        return FlowHelmMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: FlowHelmPedometerParams) async throws -> FlowHelmPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<FlowHelmPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = FlowHelmPedometerPayload(
