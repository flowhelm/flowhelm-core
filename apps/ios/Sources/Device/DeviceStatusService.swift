...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> FlowHelmDeviceStatusPayload {
        ...
        return FlowHelmDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> FlowHelmDeviceInfoPayload {
        ...
        return FlowHelmDeviceInfoPayload(
    ...
    private func batteryStatus() -> FlowHelmBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: FlowHelmBatteryState = switch device.batteryState {
        ...
        return FlowHelmBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> FlowHelmThermalStatusPayload {
        let state: FlowHelmThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return FlowHelmThermalStatusPayload(state: state)
    }

    private func storageStatus() -> FlowHelmStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return FlowHelmStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
