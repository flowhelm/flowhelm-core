...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> FlowHelmNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> FlowHelmNetworkStatusPayload {
        let status: FlowHelmNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> FlowHelmNetworkStatusPayload {
        FlowHelmNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
