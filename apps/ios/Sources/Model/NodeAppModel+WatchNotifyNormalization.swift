...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: FlowHelmWatchNotifyParams) -> FlowHelmWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [FlowHelmWatchAction]?,
        kind: String?,
        promptId: String?) -> [FlowHelmWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> FlowHelmWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return FlowHelmWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                FlowHelmWatchAction(id: "approve", label: "Approve"),
                FlowHelmWatchAction(id: "decline", label: "Decline", style: "destructive"),
                FlowHelmWatchAction(id: "open_phone", label: "Open iPhone"),
                FlowHelmWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            FlowHelmWatchAction(id: "done", label: "Done"),
            FlowHelmWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            FlowHelmWatchAction(id: "open_phone", label: "Open iPhone"),
            FlowHelmWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: FlowHelmWatchRisk?,
        priority: FlowHelmNotificationPriority?) -> FlowHelmWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: FlowHelmNotificationPriority?,
        risk: FlowHelmWatchRisk?) -> FlowHelmNotificationPriority?
    {
