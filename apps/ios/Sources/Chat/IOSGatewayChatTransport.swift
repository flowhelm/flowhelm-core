...
struct IOSGatewayChatTransport: FlowHelmChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.flowhelm", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> FlowHelmChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(FlowHelmChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> FlowHelmChatHistoryPayload {
        ...
        return try JSONDecoder().decode(FlowHelmChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [FlowHelmChatAttachmentPayload]) async throws -> FlowHelmChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [FlowHelmChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(FlowHelmChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(FlowHelmGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FlowHelmGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FlowHelmChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FlowHelmAgentEventPayload.self)
                        {
