...
@Suite(.serialized) struct GatewayConnectionSecurityTests {
    private func clearTLSFingerprint(stableID: String) {
        let suite = UserDefaults(suiteName: "ai.flowhelm.shared") ?? .standard
    ...
    @Test @MainActor func discoveredTLSParams_prefersStoredPinOverAdvertisedTXT() async {
        ...
        let endpoint: NWEndpoint = .service(name: "Test", type: "_flowhelm-gw._tcp", domain: "local.", interface: nil)
    ...
    @Test @MainActor func manualConnectionsForceTLSForNonLoopbackHosts() async {
        ...
        #expect(controller._test_resolveManualUseTLS(host: "flowhelm.local", useTLS: false) == true)
