...
```bash
flowhelm plugins install @flowhelm/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
flowhelm plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "FlowHelm" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `flowhelm pairing list nextcloud-talk`
  - `flowhelm pairing approve nextcloud-talk <CODE>`
