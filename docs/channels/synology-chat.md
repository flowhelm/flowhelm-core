---
summary: "Synology Chat webhook setup and FlowHelm config"
read_when:
  - Setting up Synology Chat with FlowHelm
...
```bash
flowhelm plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your FlowHelm gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in FlowHelm.
...
- Pairing approvals work with:
  - `flowhelm pairing list synology-chat`
  - `flowhelm pairing approve synology-chat <CODE>`
...
```bash
flowhelm message send --channel synology-chat --target 123456 --text "Hello from FlowHelm"
flowhelm message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
