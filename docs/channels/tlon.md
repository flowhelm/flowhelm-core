...
Tlon is a decentralized messenger built on Urbit. FlowHelm connects to your Urbit ship and can
...
```bash
flowhelm plugins install @flowhelm/tlon
```

Local checkout (when running from a git repo):

```bash
flowhelm plugins install ./extensions/tlon
```
...
By default, FlowHelm blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `flowhelm message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, FlowHelm replies in-thread.
