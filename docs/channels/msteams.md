...
```bash
flowhelm plugins install @flowhelm/msteams
```

Local checkout (when running from a git repo):

```bash
flowhelm plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
FlowHelm will offer the local install path automatically.
...
3. Configure FlowHelm with those credentials.
...
- Talk to FlowHelm via Teams DMs, group chats, or channels.
...
- On startup, FlowHelm resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.flowhelm/flowhelm.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring FlowHelm, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `flowhelm-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `flowhelm plugins install @flowhelm/msteams`
   - From a local checkout: `flowhelm plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "FlowHelm in Teams", "full": "FlowHelm in Teams" },
...
FlowHelm handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, FlowHelm only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/FlowHelmShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

FlowHelm sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `flowhelm message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.flowhelm/msteams-polls.json`.
...
```bash
flowhelm message send --channel msteams \
...
flowhelm message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
flowhelm message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
flowhelm message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
flowhelm message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
