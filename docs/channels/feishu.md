...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects FlowHelm to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
flowhelm plugins install @flowhelm/feishu
```

Local checkout (when running from a git repo):

```bash
flowhelm plugins install ./extensions/feishu
```
...
If you just installed FlowHelm, run the wizard:
...
2. Configuring app credentials in FlowHelm
...
1. You already ran `flowhelm channels add` for Feishu
2. The gateway is running (`flowhelm gateway status`)
...
```bash
flowhelm pairing approve feishu <CODE>
```
...
2. Run `flowhelm logs --follow` and look for `chat_id`
...
2. Run `flowhelm logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `flowhelm gateway status`  | Show gateway status           |
| `flowhelm gateway install` | Install/start gateway service |
| `flowhelm gateway stop`    | Stop gateway service          |
| `flowhelm gateway restart` | Restart gateway service       |
| `flowhelm logs --follow`   | Tail gateway logs             |
...
4. Check logs: `flowhelm logs --follow`
...
5. Ensure the gateway is running: `flowhelm gateway status`
6. Check logs: `flowhelm logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.flowhelm/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.flowhelm/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
