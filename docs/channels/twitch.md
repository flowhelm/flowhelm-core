...
read_when:
  - Setting up Twitch chat integration for FlowHelm
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. FlowHelm connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
flowhelm plugins install @flowhelm/twitch
```

Local checkout (when running from a git repo):

```bash
flowhelm plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `FLOWHELM_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "flowhelm", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use FLOWHELM_TWITCH_ACCESS_TOKEN env var)
...
```bash
FLOWHELM_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
