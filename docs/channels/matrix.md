...
Matrix is an open, decentralized messaging protocol. FlowHelm connects as a Matrix **user**
...
```bash
flowhelm plugins install @flowhelm/matrix
```

Local checkout (when running from a git repo):

```bash
flowhelm plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `flowhelm plugins install @flowhelm/matrix`
   - From a local checkout: `flowhelm plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: FlowHelm calls the same
     login endpoint, stores the access token in `~/.flowhelm/credentials/matrix/credentials.json`,
...
- On first connection, FlowHelm requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  FlowHelm logs a warning.
...
`~/.flowhelm/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `flowhelm pairing list matrix`
  - `flowhelm pairing approve matrix <CODE>`
...
- On startup, FlowHelm resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
