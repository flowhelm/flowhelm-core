---
summary: "Mattermost bot setup and FlowHelm config"
read_when:
...
```bash
flowhelm plugins install @flowhelm/mattermost
```

Local checkout (when running from a git repo):

```bash
flowhelm plugins install ./extensions/mattermost
```
...
4. Configure FlowHelm and start the gateway.
...
- Approve via:
  - `flowhelm pairing list mattermost`
  - `flowhelm pairing approve mattermost <CODE>`
...
Use these target formats with `flowhelm message send` or cron/webhooks:
