...
description: "Complete field-by-field reference for ~/.flowhelm/flowhelm.json"
summary: "Complete reference for every FlowHelm config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.flowhelm/flowhelm.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — FlowHelm uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `flowhelm doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["flowhelm-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `flowhelm plugins install @flowhelm/mattermost`.
...
FlowHelm spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@flowhelm"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `flowhelm.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, FlowHelm auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, FlowHelm assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "flowhelm-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.flowhelm/sandboxes`
...
noVNC observer access uses VNC auth by default and FlowHelm emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `flowhelm-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.flowhelm/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@flowhelm"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:flowhelm`   | All built-in tools (excludes provider plugins)                                           |
...
FlowHelm uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.flowhelm/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `FLOWHELM_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `flowhelm onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `flowhelm onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `flowhelm onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `flowhelm onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `flowhelm onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `flowhelm onboard --auth-choice minimax-api`.
...
- Loaded from `~/.flowhelm/extensions`, `<workspace>/.flowhelm/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or FLOWHELM_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `FLOWHELM_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.flowhelm-dev` + port `19001`), `--profile <name>` (uses `~/.flowhelm-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.flowhelm/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-flowhelm-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `FLOWHELM_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or FLOWHELM_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__flowhelm__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__flowhelm__/a2ui/`
...
- Also serves A2UI at `/__flowhelm__/a2ui/`.
...
- Hostname defaults to `flowhelm`. Override with `FLOWHELM_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.flowhelm/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `flowhelm dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.flowhelm/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${FLOWHELM_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.flowhelm/credentials/oauth.json`.
...
- Default log file: `/tmp/flowhelm/flowhelm-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `flowhelm doctor --fix` can strip unknown keys).
