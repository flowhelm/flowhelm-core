...
- During heartbeat runs, FlowHelm treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `flowhelm sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, FlowHelm skips the heartbeat run entirely (no model call).
...
```bash
flowhelm system event --text "Check for urgent follow-ups" --mode now
```
