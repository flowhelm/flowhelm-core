...
read_when:
  - Running FlowHelm behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of FlowHelm
...
- You run FlowHelm behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. FlowHelm checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. FlowHelm extracts the user identity from the configured header
...
- FlowHelm can stay on loopback HTTP behind the proxy.
...
If FlowHelm itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://flowhelm.example.com
    to: http://flowhelm-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
flowhelm.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy flowhelm:18789 {
...
location / {
    ...
    proxy_pass http://flowhelm:18789;
...
`flowhelm security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update FlowHelm config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `flowhelm security audit` and review findings
