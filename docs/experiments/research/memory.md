...
read_when:
  - Designing workspace memory (~/.flowhelm/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep FlowHelm integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.flowhelm/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.flowhelm/workspace` as canonical human-readable memory.
...
```
~/.flowhelm/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in FlowHelm**, but keep a separable core library.

### Why integrate into FlowHelm?

- FlowHelm already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `flowhelm memory recall "…" --k 25 --since 30d`
  - `flowhelm memory reflect --since 7d`
...
Pragmatic take for `~/.flowhelm/workspace`:
