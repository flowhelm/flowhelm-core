...
- Publish as `flowhelm/plugin-sdk` (or export from core under `flowhelm/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `FlowHelmPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: FlowHelmConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: FlowHelmConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: FlowHelmConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: FlowHelmConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: FlowHelmConfig): string;
  };
};
```
...
- Introduce `flowhelm/plugin-sdk`.
- Add `api.runtime` to `FlowHelmPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `flowhelmRuntime: ">=2026.2.0"`).
