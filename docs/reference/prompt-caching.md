...
- With Anthropic API-key auth profiles, FlowHelm seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, FlowHelm injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
FlowHelm exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.flowhelm/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$FLOWHELM_STATE_DIR/logs/cache-trace.jsonl`
...
- `FLOWHELM_CACHE_TRACE=1` enables cache tracing.
- `FLOWHELM_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `FLOWHELM_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `FLOWHELM_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `FLOWHELM_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
