...
FlowHelm features that can generate provider usage or paid API calls.
...
- `flowhelm status --usage` and `flowhelm channels list` show provider **usage windows**
...
FlowHelm can pick up credentials from:
...
`flowhelm models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
