...
FlowHelm integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- FlowHelm owns lifecycle when `channels.signal.autoStart=true`.
...
- FlowHelm spawns `imsg rpc` as a child process (legacy iMessage integration).
