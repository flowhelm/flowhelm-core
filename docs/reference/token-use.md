---
summary: "How FlowHelm builds prompt context and reports token usage + costs"
read_when:
...
FlowHelm tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

FlowHelm assembles its own system prompt on every run. It includes:
...
For images, FlowHelm downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from FlowHelm session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `flowhelm status --usage` and `flowhelm channels list` show
...
`cacheWrite`. If pricing is missing, FlowHelm shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. FlowHelm can
...
Anthropic's 1M context window is currently beta-gated. FlowHelm can inject the
...
FlowHelm skips the `context-1m-*` beta header because Anthropic currently
