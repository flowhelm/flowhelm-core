---
summary: "Use Venice AI privacy-focused models in FlowHelm"
read_when:
  - You want privacy-focused inference in FlowHelm
...
```bash
flowhelm onboard --auth-choice venice-api-key
```
...
```bash
flowhelm agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, FlowHelm shows all available Venice models. Pick based on your needs:
...
```bash
flowhelm models set venice/claude-opus-45
flowhelm models set venice/llama-3.3-70b
```

List all available models:

```bash
flowhelm models list | grep venice
```

## Configure via `flowhelm configure`

1. Run `flowhelm configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
FlowHelm automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
flowhelm agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
flowhelm agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
flowhelm agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
flowhelm agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
flowhelm agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `flowhelm models list` to see currently available models. Some models may be temporarily offline.
