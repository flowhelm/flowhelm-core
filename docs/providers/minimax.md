---
summary: "Use MiniMax M2.1 in FlowHelm"
read_when:
  - You want MiniMax models in FlowHelm
...
```bash
flowhelm plugins enable minimax-portal-auth  # skip if already loaded.
flowhelm gateway restart  # restart if gateway is already running
flowhelm onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/flowhelm/flowhelm/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `flowhelm.json`:
...
- Use `flowhelm models list` and `flowhelm models set minimax/MiniMax-M2.1` to switch.
...
- Running `flowhelm configure` and selecting **MiniMax M2.1**, or
