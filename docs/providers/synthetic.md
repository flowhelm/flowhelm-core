---
summary: "Use Synthetic's Anthropic-compatible API in FlowHelm"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. FlowHelm registers it as the
...
Note: FlowHelm's Anthropic client appends `/v1` to the base URL, so use
