...
read_when:
  - You want to place an outbound voice call from FlowHelm
...
Voice calls for FlowHelm via a plugin. Supports outbound notifications and
...
- Use `flowhelm voicecall ...` or the `voice_call` tool
...
```bash
flowhelm plugins install ./extensions/voice-call
...
```bash
flowhelm voicecall call --to "+15555550123" --message "Hello from FlowHelm"
flowhelm voicecall continue --call-id <id> --message "Any questions?"
flowhelm voicecall speak --call-id <id> --message "One moment"
flowhelm voicecall end --call-id <id>
flowhelm voicecall status --call-id <id>
flowhelm voicecall tail
