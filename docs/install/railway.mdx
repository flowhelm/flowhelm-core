...
Deploy FlowHelm on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/flowhelm_bot-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/flowhelm` — Control UI

## What you get

- Hosted FlowHelm Gateway + Control UI
...
- `FLOWHELM_STATE_DIR=/data/.flowhelm` (recommended)
- `FLOWHELM_WORKSPACE_DIR=/data/workspace` (recommended)
- `FLOWHELM_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your FlowHelm state + workspace so you can migrate to another host without losing config or memory.
