...
Deploy FlowHelm on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/flowhelm/flowhelm)
...
repository configures everything needed to run FlowHelm:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: FLOWHELM_STATE_DIR
        value: /data/.flowhelm
      - key: FLOWHELM_WORKSPACE_DIR
        value: /data/workspace
      - key: FLOWHELM_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/flowhelm`.
...
If you use the original FlowHelm repository, Render will not auto-deploy your FlowHelm. To update it, run a manual Blueprint sync from the dashboard.
...
For FlowHelm, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any FlowHelm host.
