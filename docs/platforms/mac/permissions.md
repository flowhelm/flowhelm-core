...
- Same path: run the app from a fixed location (for FlowHelm, `dist/FlowHelm.app`).
...
Workaround: move files into the FlowHelm workspace (`~/.flowhelm/workspace`) if you want to avoid per-folder grants.
