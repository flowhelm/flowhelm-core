...
service via the external `flowhelm` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`FLOWHELM_PROFILE`; legacy `com.flowhelm.*` is supported).
