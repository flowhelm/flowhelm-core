---
summary: "FlowHelm on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up FlowHelm on a Raspberry Pi
  - Running FlowHelm on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# FlowHelm on Raspberry Pi

## Goal

Run a persistent, always-on FlowHelm Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most FlowHelm features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled flowhelm

# Enable if not
sudo systemctl enable flowhelm

# Start on boot
sudo systemctl start flowhelm
```
...
```bash
# Check logs
journalctl -u flowhelm --no-pager -n 100

# Common fix: rebuild
cd ~/flowhelm  # if using hackable install
npm run build
sudo systemctl restart flowhelm
```
