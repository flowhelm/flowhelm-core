...
你可以在 `~/.flowhelm/flowhelm.json` 中覆盖此设置：
...
所有日志配置都在 `~/.flowhelm/flowhelm.json` 的 `logging` 下。
...
- FlowHelm 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `flowhelm plugins enable diagnostics-otel` 启用插件。
...
- `flowhelm.tokens`（计数器，属性：`flowhelm.token`、`flowhelm.channel`、`flowhelm.provider`、`flowhelm.model`）
- `flowhelm.cost.usd`（计数器，属性：`flowhelm.channel`、`flowhelm.provider`、`flowhelm.model`）
- `flowhelm.run.duration_ms`（直方图，属性：`flowhelm.channel`、`flowhelm.provider`、`flowhelm.model`）
- `flowhelm.context.tokens`（直方图，属性：`flowhelm.context`、`flowhelm.channel`、`flowhelm.provider`、`flowhelm.model`）

消息流：

- `flowhelm.webhook.received`（计数器，属性：`flowhelm.channel`、`flowhelm.webhook`）
- `flowhelm.webhook.error`（计数器，属性：`flowhelm.channel`、`flowhelm.webhook`）
- `flowhelm.webhook.duration_ms`（直方图，属性：`flowhelm.channel`、`flowhelm.webhook`）
- `flowhelm.message.queued`（计数器，属性：`flowhelm.channel`、`flowhelm.source`）
- `flowhelm.message.processed`（计数器，属性：`flowhelm.channel`、`flowhelm.outcome`）
- `flowhelm.message.duration_ms`（直方图，属性：`flowhelm.channel`、`flowhelm.outcome`）

队列 + 会话：

- `flowhelm.queue.lane.enqueue`（计数器，属性：`flowhelm.lane`）
- `flowhelm.queue.lane.dequeue`（计数器，属性：`flowhelm.lane`）
- `flowhelm.queue.depth`（直方图，属性：`flowhelm.lane` 或 `flowhelm.channel=heartbeat`）
- `flowhelm.queue.wait_ms`（直方图，属性：`flowhelm.lane`）
- `flowhelm.session.state`（计数器，属性：`flowhelm.state`、`flowhelm.reason`）
- `flowhelm.session.stuck`（计数器，属性：`flowhelm.state`）
- `flowhelm.session.stuck_age_ms`（直方图，属性：`flowhelm.state`）
- `flowhelm.run.attempt`（计数器，属性：`flowhelm.attempt`）
...
- `flowhelm.webhook.error`
  - `flowhelm.channel`、`flowhelm.webhook`、`flowhelm.chatId`、`flowhelm.error`
- `flowhelm.message.processed`
  - `flowhelm.channel`、`flowhelm.outcome`、`flowhelm.chatId`、`flowhelm.messageId`、`flowhelm.sessionKey`、`flowhelm.sessionId`、`flowhelm.reason`
- `flowhelm.session.stuck`
  - `flowhelm.state`、`flowhelm.ageMs`、`flowhelm.queueDepth`、`flowhelm.sessionKey`、`flowhelm.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `flowhelm doctor`。
