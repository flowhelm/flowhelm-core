...
- `gateway`/`node` approvals are controlled by `~/.flowhelm/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  FlowHelm prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`flowhelm security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `flowhelm doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
