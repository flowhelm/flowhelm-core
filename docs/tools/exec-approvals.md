...
- **gateway host** → `flowhelm` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.flowhelm/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.flowhelm/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `flowhelm approvals allowlist ...`).
- `flowhelm security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `flowhelm doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.flowhelm/exec-approvals.json` directly.

CLI: `flowhelm approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
