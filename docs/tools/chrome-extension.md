---
summary: "Chrome extension: let FlowHelm drive your existing Chrome tab"
read_when:
...
The FlowHelm Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate flowhelm-managed Chrome profile.
...
FlowHelm then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the FlowHelm release (npm package) as static files. There is no separate “build” step.

After upgrading FlowHelm:

- Re-run `flowhelm browser extension install` to refresh the installed files under your FlowHelm state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

FlowHelm ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `FLOWHELM_GATEWAY_TOKEN`)
...
- Open the tab you want FlowHelm to control.
...
- `ON`: attached; FlowHelm can drive that tab.
...
Debugging: `flowhelm sandbox explain`
...
`flowhelm browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `flowhelm browser extension install` first to copy the extension to a stable location under your FlowHelm state directory.
...
- **This is not isolated** like the dedicated flowhelm-managed profile.
