...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `flowhelm sandbox explain`.
...
Legacy `agent.*` configs are migrated by `flowhelm doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=flowhelm-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${FLOWHELM_STATE_DIR:-$HOME/.flowhelm}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
