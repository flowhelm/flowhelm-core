...
- `/usage cost` in chats: local cost summary aggregated from FlowHelm session logs.
- CLI: `flowhelm status --usage` prints a full per-provider breakdown.
- CLI: `flowhelm channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
