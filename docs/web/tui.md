...
```bash
flowhelm tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `flowhelm tui --deliver`
...
- Check the Gateway logs: `flowhelm logs --follow`.
- Confirm the agent can run: `flowhelm status` and `flowhelm models status`.
...
- No agents in picker: check `flowhelm agents list` and your routing config.
