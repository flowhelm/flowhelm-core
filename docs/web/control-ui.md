...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/flowhelm`)
...
If the page fails to load, start the Gateway first: `flowhelm gateway`.
...
you revoke it with `flowhelm devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.flowhelm/flowhelm.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop flowhelm`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. FlowHelm
...
```bash
flowhelm gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
FlowHelm **blocks** Control UI connections without device identity.
...
```bash
FLOWHELM_CONTROL_UI_BASE_PATH=/flowhelm/ pnpm ui:build
```
