...
- Jobs persist under `~/.flowhelm/cron/` so restarts don’t lose schedules.
...
flowhelm cron list
flowhelm cron run <job-id>
flowhelm cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.flowhelm/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `flowhelm cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, FlowHelm applies a
...
If `delivery` is omitted for isolated jobs, FlowHelm defaults to `announce`.
...
- Job store: `~/.flowhelm/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.flowhelm/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- For removed run sessions no longer referenced by the session store, FlowHelm archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `flowhelm cron runs` and adjust retention before logs become large
...
flowhelm cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
flowhelm cron edit <jobId> --agent ops
flowhelm cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
flowhelm cron run <jobId>
flowhelm cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
flowhelm cron edit <jobId> \
...
```bash
flowhelm cron edit <jobId> --exact
```
...
```bash
flowhelm system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`flowhelm system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `FLOWHELM_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- FlowHelm applies exponential retry backoff for recurring jobs after consecutive errors:
