...
read_when:
  ...
  - Wiring external systems into FlowHelm
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your FlowHelm config directory (typically `~/.flowhelm/hooks/transforms`).
...
- `flowhelm webhooks gmail setup` writes `hooks.gmail` config for `flowhelm webhooks gmail run`.
