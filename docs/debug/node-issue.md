...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in FlowHelm
...
Running FlowHelm via Node with `tsx` fails at startup with:

```
[flowhelm] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `flowhelm status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch flowhelm.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node flowhelm.mjs status` works on Node 25.
